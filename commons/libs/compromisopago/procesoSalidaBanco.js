...
var generarCabecera  = function GenerarCabacera(numVersion,fecha){
  ...
  linea = linea + '000'; //Número proporcionado por el banco
    ...
    CompromisoPago.find({
      ...
      if (err) {
        ...
        items.forEach(function(itemtabla) {
          ...
          cadtemp = utils.RemplazaCaracteres(itemtabla._persona.nombreCompleto) + vacios.substr(0, 30);
          linea = linea + cadtemp.replace(/-/g, ' ').substr(0, 30); //Nombre del Cliente
          cadtemp = itemtabla._persona.documento + ' ';
          cadtemp = utils.pad(cadtemp.trim(),8,'0');
          //cadtemp = cadtemp.substr(-8) + ' ' + itemtabla.codigo + ' ' + utils.RemplazaCaracteres(itemtabla._tasa.nombre) + vacios.substr(0, 48);
          cadtemp = (cadtemp.substr(-8) + ' ' + itemtabla.codigo + ' ' + utils.RemplazaCaracteres(itemtabla._tasa.nombre)).substr(0, 48);
          linea = linea + cadtemp.substr(0, 48); //Campo de Identificación del Pago
